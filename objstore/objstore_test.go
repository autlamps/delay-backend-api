...
// TODO: Review this get function maybe we should be doing it differently
func TestService_Get(t *testing.T) {
	tests := []struct {
		...
		{"test", `"{hello: "world"}`, 0, nil},
		{"test2", `"{hello: "world"}`, 2, ErrFailedToGetKey},
	}
	...
	for _, test := range tests {
		...
		bv, err := s.Get(test.key)

		if err != test.err {
			t.Fatalf("%v - Errors dont match: Expected %v, got %v", test.key, test.err, err)

		}

		if test.err != ErrFailedToGetKey {
			bs := string(bv)

			if bs != test.value {
				t.Errorf("%v - Values differ from saved: Expected: %v, got %v", test.key, test.value, bs)
			}
		}
	}
}
